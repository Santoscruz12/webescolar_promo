<div class="bodyps">
  <div class="headeps">
    <h1 class="titulops">Módulos</h1>
    <div class="containerps">
      <hr>

    </div>
  </div>

  <div class="contenidops">
    <div class="contenido_textops">
      <h1 class="conte_titulops">PORTAL DE
        SOLUCIONES</h1>

      <ul class="lista-solucionesps">
        <li class="item-solucionps">
          <img class="contenido_texto_imaps" src="./assets/image/ecofile-removebg-preview.png" alt="ecofile">
          <img class="contenido_iconoap" src="./assets/image/138.png" alt="icono viñeta">
          <div class="bloque-texto-viñeta">
            <div class="linea-icono-texto">

              <strong class="contenido_tips">Factura Electrónica Automática</strong>
            </div>
            <p>
              Genera facturas válidas ante el SAT de forma automática y sin errores. Todo desde una plataforma integral, fácil de usar y 100% en línea.
            </p>
          </div>
        </li>

        <li class="item-solucionps">
          <img class="contenido_texto_imaps2" src="./assets/image/AXS_Control-removebg-preview.png" alt="axs">
          <img class="contenido_iconoap" src="./assets/image/138.png" alt="icono viñeta">
          <div class="bloque-texto-viñeta">
            <div class="linea-icono-texto">

              <strong class="contenido_tips">Control de Acceso con Código QR</strong>
            </div>
            <p>
              Gestiona entradas y salidas con tecnología QR. Seguridad, rapidez y cero filas desde cualquier dispositivo.
            </p>
          </div>
        </li>
      </ul>
      <div class="boton_ap item1p">
        <a href="#contactanos"><button class="conte_buttonap">AGENDAR DEMO</button></a>

      </div>
    </div>


    <div class="contenidosps item2p">
      <div class="conten_imagenesps">
        <div class="imagen-con-textops">
          <img class="conten_imagenes_fotops" src="./assets/image/Servicios Integrados_carrusel.png"
            alt="Portal de solucionesps">
          <div class="texto-imagenps">PORTAL DE SOLUCIONES</div>
        </div>

        <div class="imagen-con-textops">
          <img class="conten_imagenes_fotops" src="./assets/image/Servicios Escolares_carrusel.png"
            alt="Servicios integradosps">
          <div class="texto-imagenps">SERVICIOS ESCOLARES</div>
        </div>

        <div class="imagen-con-textops">
          <img class="conten_imagenes_fotops" src="./assets/image/servisios academicos_carrusel.png"
            alt="Servicios escolaresps">
          <div class="texto-imagenps">SERVICIOS ACADÉMICOS</div>
        </div>
      </div>
      <br>

      <div class="conten_iconosps">

        <a href="#" onclick="navegarCarrusel(-1)">
          <img class="conten_iconos_icops" src="./assets/image/flecha_izquierda.png" alt="Iconos">

        </a>

        <a href="#" onclick="navegarCarrusel(1)">
          <img class="conten_iconos_icops" src="./assets/image/flecha_derecha.png" alt="Iconos">

        </a>

        <hr class="conten_iconos_hrps">

        <p class="conten_iconos_numps">02</p>







      </div>




    </div>







  </div>

</div>