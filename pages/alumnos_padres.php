<div class="bodyap">
    <div class="headeap">
        <h1 class="tituloap">Modulos</h1>
        <div class="containerap">

            
                <hr>
          
            <hr>
        </div>
    </div>

    <div class="contenidoap">
        <div class="contenido_textoap">
            <h1 class="conte_tituloap">ALUMNOS Y PADRES</h1>
            <h2 class="conte_titulo_gesap">GESTIÓN ESCOLAR DIGITAL AL ALCANCE DE UN CLIC</h2>

            <ul>
                <li>
                    <img class="contenido_iconoap" src="./assets/image/138.png" alt="logo">
                    <div class="textoap">
                        <strong>Pago en línea de colegiaturas </strong><br>
                        Realiza tus pagos escolares fácilmente con tarjeta de crédito, débito o en Oxxo,
                        disponible 24/7.
                    </div>
                </li>

                <li>
                    <img class="contenido_iconoap" src="./assets/image/138.png" alt="logo">
                    <div class="textoap">
                        <strong>Consulta de historial académico</strong><br>
                        Accede en tiempo real a tus calificaciones, materias cursadas y avances
                        académicos
                    </div>
                </li>

                <li>
                    <img class="contenido_iconoap" src="./assets/image/138.png" alt="logo">
                    <div class="textoap">
                        <strong>Caja y facturación electrónica</strong><br>
                        Genera fichas de pago, consulta tu estado de cuenta y descarga facturas al
                        instante
                    </div>
                </li>

                <li>
                    <img class="contenido_iconoap" src="./assets/image/138.png" alt="logo">
                    <div class="textoap">
                        <strong>Evaluación docente en línea</strong><br>
                        Responde encuestas de profesores y obtén reportes automatizados por materia
                        y docente.
                    </div>
                </li>

            </ul>
        </div>


        <div class="contenidosap">
            <div class="conten_imagenesap">

                <div class="imagen-con-textoap">
                    <img class="conten_imagenes_fotoap" src="./assets/image/Portal_Soluciones_carrusel.png"
                        alt="Portal de solucionesap">
                    <div class="texto-imagenap">PORTAL DE SOLUCIONES</div>
                </div>

                <div class="imagen-con-textoap">
                    <img class="conten_imagenes_fotoap" src="./assets/image/Servicios Integrados_carrusel.png"
                        alt="Servicios integradosap">
                    <div class="texto-imagenap">SERVICIOS INTEGRADOS</div>
                </div>

                <div class="imagen-con-textoap">
                    <img class="conten_imagenes_fotoap" src="./assets/image/Servicios Escolares_carrusel.png"
                        alt="Servicios escolaresap">
                    <div class="texto-imagenap">SERVICIOS ESCOLARES</div>
                </div>







            </div>
            <br>

            <div class="conten_iconosap">


                <img class="conten_iconos_icoap" src="./assets/image/flecha_izquierda.png" alt="Iconos">



                <a href="#" onclick="navegarCarrusel(1)">
                    <img class="conten_iconos_icoap" src="./assets/image/flecha_derecha.png" alt="Iconos">

                </a>


                <hr class="conten_iconos_hrap">

                <p class="conten_iconos_numap">01</p>









            </div>
            <div class="boton_ap">
                <a href="#contactanos"><button class="conte_buttonap">AGENDAR DEMO</button></a>

            </div>



        </div>







    </div>

</div>